...
const toList = (value, fallback = []) => {
  if (Array.isArray(value)) {
    return value
      .map((item) =>
        typeof item === "string" ? item.trim() : item?.name || item?.label || ""
      )
      .filter(Boolean);
  }
    ...
    return (
      <img
        ...
        className={`${className} rounded-full object-cover ${
          ring
            ? "border-4 border-white shadow-sm dark:border-[#0b1220]"
  ...
  return (
    <div
      ...
      className={`${className} flex items-center justify-center rounded-full bg-teal-700 font-black tracking-tight text-white ${
        ring ? "border-4 border-white shadow-sm dark:border-[#0b1220]" : ""
      }`}
    >
      {getInitials(name)}
    </div>
...
const MyProfile = () => {
  ...
  const coverPhotoURL = cleanText(
    userInfo?.coverPhotoURL || userInfo?.coverPhoto,
    ""
  );
  const bio = cleanText(
  ...
  const isOnline = Boolean(
    userInfo?.isOnline || userInfo?.online || userInfo?.onlineStatus === "online"
  );
  ...
  const learning = useMemo(() => {
    ...
    return {
      ...
      weeklyGoal: cleanText(
        String(userInfo?.weeklyGoal ?? stats?.weeklyGoal ?? "0/7"),
        "0/7"
      ),
    ...
    if (Array.isArray(raw) && raw.length) {
      return raw
        .map((item) => ({
          ...
          value: Math.max(
            0,
            Math.min(100, safeNumber(item?.value ?? item?.progress, 0))
          ),
        }))
    ...
    return (
      <main className="min-h-screen w-full bg-slate-50 dark:bg-[#050713]">
        ...
        <div className="w-full px-5 pb-10 sm:px-8 lg:px-10">
          ...
          <div className="mt-5 h-8 w-52 rounded-lg bg-slate-200 dark:bg-white/[0.06]" />
          <div className="mt-3 h-4 w-72 max-w-full rounded-lg bg-slate-100 dark:bg-white/[0.04]" />
        </div>
      </main>
    );
  }
  ...
  if (error || !userInfo) {
    return (
      <main className="flex min-h-screen w-full items-center justify-center bg-slate-50 px-4 dark:bg-[#050713]">
    ...
    <main className="min-h-screen w-full overflow-x-hidden bg-slate-50 text-slate-950 dark:bg-[#050713] dark:text-white">
      ...
      <section className="w-full border-b border-slate-200 bg-white dark:border-white/10 dark:bg-[#0b1220]">
        <div className="relative h-56 w-full overflow-hidden bg-[#082f36] sm:h-72 lg:h-[340px]">
          ...
          <div className="absolute left-5 top-5 flex flex-wrap items-center gap-2 sm:left-8 sm:top-7 lg:left-10">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-slate-950/40 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-white">
        ...
        <div className="w-full px-5 pb-7 sm:px-8 sm:pb-8 lg:px-10">
          ...
          <div className="mt-7 grid grid-cols-3 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/[0.03]">
            {statConfig.map((item, index) => (
              ...
              >
                <span className="hidden h-10 w-10 items-center justify-center rounded-xl bg-white text-slate-400 shadow-sm group-hover:text-teal-700 dark:bg-white/[0.05] dark:group-hover:text-teal-300 sm:flex">
                  <i className={`fa-solid ${item.icon} text-xs`} aria-hidden="true" />
                </span>
                <span>
                  <span className="block text-xl font-black leading-none text-slate-950 dark:text-white sm:text-2xl">
      ...
      <div className="grid w-full gap-5 px-4 py-5 sm:px-6 sm:py-6 lg:grid-cols-[320px_minmax(0,1fr)] lg:px-8 xl:grid-cols-[360px_minmax(0,1fr)] xl:px-10">
        ...
        <div className="min-w-0 space-y-5">
          <section className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#101626] sm:p-7">
            <SectionHeader
              icon="fa-chart-simple"
              title="Learning snapshot"
              subtitle="A quick view of speaking progress"
            />
          ...
          <section className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#101626] sm:p-7">
            <SectionHeader
              ...
              subtitle="Current learning progress by language"
            />
          ...
          <section className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#101626] sm:p-7">
            <SectionHeader
              icon="fa-award"
              title="Achievements"
              subtitle="Milestones earned on Vaani"
            />
          ...
          <section className="rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#101626] sm:p-7">
            <SectionHeader
              ...
              subtitle="Latest learning and community moments"
            />
              ...
              <div className="grid gap-3 xl:grid-cols-2">
                {recentActivity.map((activity, index) => (
                  <div
                    ...
                    className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/[0.03]"
                  >
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-slate-500 dark:bg-white/[0.05] dark:text-slate-300">
        ...
        >
          <div className="flex max-h-[82vh] w-full max-w-lg flex-col overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-xl dark:border-white/10 dark:bg-[#101626]">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/[0.07] sm:px-6">
              <div>
                <h3 className="text-lg font-black capitalize text-slate-950 dark:text-white">
                  {popupType}
                </h3>
            ...
            <div className="border-b border-slate-100 bg-slate-50 px-5 py-3 dark:border-white/[0.07] dark:bg-white/[0.02] sm:px-6">
              <div className="relative">
                <i
                  className="fa-solid fa-search pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-xs text-slate-400"
                  aria-hidden="true"
                />
                <input
                  type="search"
